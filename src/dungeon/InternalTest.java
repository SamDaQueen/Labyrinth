...
public class InternalTest {
  ...
  public void getEnd() {
    dungeonFixed = new DungeonImpl(new int[]{5, 5}, 6, false, 20, getCaves(), new int[]{4, 3},
        new int[]{0, 1});
    assertEquals(0, ((DungeonImpl) dungeonFixed).getEnd()[0]);
    assertEquals(1, ((DungeonImpl) dungeonFixed).getEnd()[1]);
  }
  ...
  public void checkMinimumPath() {
    for (int i = 0; i < 1000; i++) {
      ...
      assertTrue(
          abs(dungeon.getStart()[0] - dungeon.getEnd()[0]) + abs(dungeon.getStart()[1]
              - dungeon.getEnd()[1])
              >= 5);
    }
  }
  ...
  public void allTreasure() {
    dungeon = new DungeonImpl(new int[]{5, 5}, 0, false, 100);
    boolean all = true;
    for (int row = 0; row < 5; row++) {
      for (int col = 0; col < 5; col++) {
        if (!dungeon.getDungeon()[row][col].isTunnel()) {
          if (dungeon.getDungeon()[row][col].getTreasure().size() < 1) {
            all = false;
          }
        }
      }
  ...
  public void printPlayerStatus() {
    ...
    assertEquals("Player has collected DIAMOND(1), SAPPHIRE(2), RUBY(1),  "
            + "with a score of 90",
  ...
  public void numberOfCaves() {
    dungeonFixed = new DungeonImpl(new int[]{5, 5}, 3, false, 20, getCaves(), new int[]{4, 3},
        new int[]{0, 1});
    assertEquals(16, ((DungeonImpl) dungeonFixed).getNumberOfCaves());
  }
}
